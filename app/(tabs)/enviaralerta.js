...
const EnviarAlerta = () => {
  ...
  const mercalliScales = [
    { value: "1", label: "I - Muy débil", desc: "No se siente, solo detectado por instrumentos" },
    { value: "2", label: "II - Débil", desc: "Sentido por pocas personas en reposo" },
    { value: "3", label: "III - Leve", desc: "Sentido por personas en interior de edificios" },
    { value: "4", label: "IV - Moderado", desc: "Objetos colgantes oscilan visiblemente" },
    { value: "5", label: "V - Poco fuerte", desc: "Se despiertan las personas dormidas" },
    { value: "6", label: "VI - Fuerte", desc: "Caminar es difícil, objetos caen" },
    { value: "7", label: "VII - Muy fuerte", desc: "Daños en edificios mal construidos" },
    { value: "8", label: "VIII - Destructivo", desc: "Daños considerables en estructuras" },
    { value: "9", label: "IX - Ruinoso", desc: "Pánico general, daños graves" },
    { value: "10", label: "X - Desastroso", desc: "Destrucción de edificios" },
    { value: "11", label: "XI - Muy desastroso", desc: "Pocas estructuras quedan en pie" },
    { value: "12", label: "XII - Catastrófico", desc: "Destrucción total, ondas visibles" }
  ];
  ...
  const handleEnviarAlerta = async () => {
    ...
    setLoading(true);
    const { hora, fecha } = getCurrentDateTime();
    ...
    try {
      const response = await fetch("https://vortexcode.cl/api/alerta/crear-alerta", {
        method: "POST",
        headers: {
    ...
    <View className="flex-1 bg-gray-900 p-6">
      ...
      <View className="bg-gray-800 rounded-lg p-4 mb-6">
            ...
            {mercalliScales.map((scale) => (
              <Picker.Item
                ...
                style={{
                  ...
                  color: scale.value <= 3 ? '#4ADE80' :  // verde para niveles 1-3
                         scale.value <= 6 ? '#EAB308' :  // amarillo para niveles 4-6
                         scale.value <= 9 ? '#F97316' :  // naranja para niveles 7-9
        ...
        <View className={`mt-4 p-3 rounded-lg ${
          parseInt(magnitud) <= 3 ? 'bg-green-500' :
          parseInt(magnitud) <= 6 ? 'bg-yellow-500' :
          parseInt(magnitud) <= 9 ? 'bg-orange-500' :
          'bg-red-500'
        }`}>
